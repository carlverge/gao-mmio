...
static irqreturn_t e1000_msix_other(int irq, void *data)
{
	...
	if (!(icr & E1000_ICR_INT_ASSERTED)) {
		...
		//E1000E Bug: If RXO is asserted but masked, the other handler
		//is flooded with interrupts that do not have INT_ASSERTED set.
		//This happens when there is a significant receive overflow.
		//If RXO is deasserted in this case it shuts up.
		if(icr & E1000_ICR_RXO) ew32(ICR, E1000_ICR_RXO);
		return IRQ_NONE;
	}
...
#ifdef GAO_E1000E_H_
	if(adapter->netdev->priv_flags & IFF_GAO_ENABLED) {
		gao_e1000e_activate_port(adapter->netdev);
	}
#endif
...
#ifdef GAO_E1000E_H_
	gao_e1000e_deactivate_port(netdev);
#endif
...
#ifdef GAO_E1000E_H_
	gao_e1000e_register_port(netdev);
#endif
...
#ifdef GAO_E1000E_H_
	gao_e1000e_unregister_port(netdev);
#endif
